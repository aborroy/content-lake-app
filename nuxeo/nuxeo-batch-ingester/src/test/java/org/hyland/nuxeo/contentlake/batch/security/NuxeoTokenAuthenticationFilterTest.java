...
class NuxeoTokenAuthenticationFilterTest {
    ...
    void authenticatesRequestFromNuxeoTokenHeader() throws Exception {
        ...
        var authenticated = new UsernamePasswordAuthenticationToken("nuxeo-user", null);
        when(authenticationManager.authenticate(any())).thenReturn(authenticated);

        NuxeoTokenAuthenticationFilter filter = new NuxeoTokenAuthenticationFilter(authenticationManager);
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/api/sync/batch");
        ...
        assertThat(captor.getValue().getName())
                .isEqualTo(NuxeoTokenAuthenticationProvider.NUXEO_TOKEN_PRINCIPAL_PREFIX + "token-123");
