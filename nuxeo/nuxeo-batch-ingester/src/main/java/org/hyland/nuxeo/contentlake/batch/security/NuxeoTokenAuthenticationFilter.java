...
/**
 * Extracts Nuxeo UI auth tokens before the Basic auth filter runs.
 */
@Slf4j
public class NuxeoTokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            String token = request.getHeader(NUXEO_TOKEN_HEADER);
            ...
            if (token != null && !token.isBlank()
                ...
                try {
                    Authentication authRequest = new UsernamePasswordAuthenticationToken(
                            NuxeoTokenAuthenticationProvider.NUXEO_TOKEN_PRINCIPAL_PREFIX + token.trim(),
                            ""
                    );
